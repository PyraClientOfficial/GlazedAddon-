...
public class GlowBerryESP extends Module {
    ...
    private final Setting<SettingColor> espColor = sgRender.add(new ColorSetting.Builder()
        ...
        .defaultValue(new SettingColor(255, 165, 0, 100)) // orange
        .build()
    );
    ...
    private final Setting<Boolean> showTracers = sgRender.add(new BoolSetting.Builder()
        .name("Show Tracers")
        .description("Draw tracer lines to glow berry vines")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Boolean> chatFeedback = sgRender.add(new BoolSetting.Builder()
        .name("Chat Feedback")
        .description("Announce glow berry vine detections in chat")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Integer> minVineLength = sgRender.add(new IntSetting.Builder()
        ...
        .description("Minimum glow berry vine length to highlight")
        .defaultValue(26)
        .min(10)
        .max(50)
        .sliderMax(50)
    ...
    private final Set<BlockPos> glowBerryVines = ConcurrentHashMap.newKeySet();

    private ExecutorService threadPool;

    public GlowBerryESP() {
        super(GlazedAddon.esp, "GlowBerryESP", "ESP for long glow berry vines.");
    }
    ...
    private void onBlockUpdate(BlockUpdateEvent event) {
        ...
        Runnable updateTask = () -> {
            boolean isGlowBerryVine = isGlowBerryVine(state, pos);

            if (isGlowBerryVine) {
                boolean wasAdded = glowBerryVines.add(pos);
                if (wasAdded && chatFeedback.get() && (!useThreading.get() || !limitChatSpam.get())) {
                    info("§6GlowBerryESP§f: Found glow berry vine at §6%s", pos.toShortString());
                }
            } else {
                glowBerryVines.remove(pos);
            }
        };
    ...
    private void scanChunk(WorldChunk chunk) {
        ...
        for (int x = xStart; x < xStart + 16; x++) {
            for (int z = zStart; z < zStart + 16; z++) {
                for (int y = yMin; y < yMax; y++) {
                    ...
                    if (isGlowBerryVineChunk(chunk, pos)) {
                        chunkVines.add(pos);
                        foundCount++;
                    }
                }
        ...
        if (chatFeedback.get() && foundCount > 0) {
            if (useThreading.get() && limitChatSpam.get()) {
                if (newVines > 0) {
                    info("§6GlowBerryESP§f: Chunk %s,%s: §6%d new vines found", cpos.x, cpos.z, newVines);
                }
            } else {
                for (BlockPos pos : chunkVines) {
                    if (!glowBerryVines.contains(pos)) {
                        info("§6GlowBerryESP§f: Found glow berry vine at §6%s", pos.toShortString());
                    }
                }
            }
    ...
    private boolean isGlowBerryVine(BlockState state, BlockPos pos) {
    ...
    private boolean isGlowBerryVineChunk(WorldChunk chunk, BlockPos pos) {
        BlockState state = chunk.getBlockState(pos);
        if (!(state.isOf(Blocks.CAVE_VINES) || state.isOf(Blocks.CAVE_VINES_PLANT))) {
            return false;
        }
    ...
    private int getVineLength(BlockPos start) {
        ...
        for (BlockPos current = start.down(); mc.world != null && (mc.world.getBlockState(current).isOf(Blocks.CAVE_VINES) || mc.world.getBlockState(current).isOf(Blocks.CAVE_VINES_PLANT)); current = current.down()) {
            length++;
        }
    ...
    private int getVineLengthChunk(WorldChunk chunk, BlockPos start) {
        ...
        for (BlockPos current = start.down(); (chunk.getBlockState(current).isOf(Blocks.CAVE_VINES) || chunk.getBlockState(current).isOf(Blocks.CAVE_VINES_PLANT)); current = current.down()) {
            length++;
        }
    ...
    private void onRender(Render3DEvent event) {
        ...
        for (BlockPos pos : glowBerryVines) {
